...
local function SMS4_Update_block(rk, _in, _out, enc)
    ...
    return string.format("%08x", X[3]) ..
            string.format("%08x", X[2]) ..
            string.format("%08x", X[1]) ..
            string.format("%08x", X[0])
...
function _M.encrypt(self, text)
    ...
    for j = 1, n do
        ...
        --local abc = str.to_hex(string.sub(ffi_str(_out), 1, 16))
        local sm4str = SMS4_Update_block(self.rk, _in, _out, true)
        table.insert(data, sm4str)
    end
